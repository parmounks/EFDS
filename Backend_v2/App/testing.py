...
def fetch_user_locations():
    ...
    conn.close()
    logging.info(f"🔍 User Locations Fetched: {len(user_locations)} users")
...
def fetch_fire_data():
    ...
    try:
        ...
        if df.empty:
            logging.warning("⚠️ No fire data found!")
        ...
        generate_fire_map(pd.DataFrame())

# Function to generate the interactive fire map
